  ...
  static async importDosen(csvText: string, mode: string = 'skip'): Promise<ImportResult> {
    ...
    for (let i = 1; i < rows.length; i++) {
      ...
      batchData.push({
        ...
        pohonIlmu: record.pohonilmu || record.pohon_ilmu || record['pohon ilmu'] || null,
        cabangIlmu: record.cabangilmu || record.cabang_ilmu || record['cabang ilmu'] || null,
        nik: record.nik || null,
            ...
            await db
              ...
              .onConflictDoUpdate({
                ...
                set: {
                  ...
                  nidn: item.nidn,
                  nuptk: item.nuptk,
                  pohonIlmu: item.pohonIlmu,
                  cabangIlmu: item.cabangIlmu,
                  nik: item.nik,
