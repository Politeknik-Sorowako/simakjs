...
export class CsvImportService {
  ...
  static async importKelasKuliahWithDosen(csvText: string, mode: string = 'skip'): Promise<ImportResult> {
    const rows = this.parseCsvLines(csvText);
    if (rows.length <= 1) {
      return { successCount: 0, errors: [{ line: 1, error: 'CSV file is empty or only has headers' }] };
    }
    ...
    const periodeIdx = headers.indexOf('periode_id');
    const namaKelasIdx = headers.indexOf('nama_kelas');
    const nipIdx = headers.indexOf('nip_dosen');
    const sksIdx = headers.indexOf('sks_beban_mengajar');
    const idPddiktiIdx = headers.indexOf('id_pddikti');

    if (kodeMkIdx === -1 || periodeIdx === -1 || namaKelasIdx === -1 || nipIdx === -1) {
      return {
        successCount: 0,
        errors: [
          {
            line: 1,
            error: 'CSV harus memiliki kolom header: kode_mata_kuliah, periode_id, nama_kelas, nip_dosen',
          },
        ],
      };
    }
    ...
    for (let i = 1; i < rows.length; i++) {
      const row = rows[i];
      const lineNum = i + 1;
      if (row.length < Math.max(kodeMkIdx, periodeIdx, namaKelasIdx, nipIdx) + 1) continue;

      const kodeMkVal = row[kodeMkIdx].trim();
      const periodeVal = row[periodeIdx].trim();
      const namaKelasVal = row[namaKelasIdx].trim();
      const nipVal = row[nipIdx].trim();
      const sksVal = sksIdx !== -1 && row[sksIdx] ? parseInt(row[sksIdx].trim()) : null;
      const idPddiktiVal = idPddiktiIdx !== -1 && row[idPddiktiIdx] ? row[idPddiktiIdx].trim() : null;

      if (!kodeMkVal || !periodeVal || !namaKelasVal || !nipVal) {
        result.errors.push({
          line: lineNum,
          error: 'Kolom kode_mata_kuliah, periode_id, nama_kelas, dan nip_dosen wajib diisi.',
        });
        continue;
      }
      ...
      try {
        ...
        if (!kelas) {
          const [newKelas] = await db
            ...
            .values({
              mataKuliahId: mk.id,
              periodeId: periodeVal,
              namaKelas: namaKelasVal,
              idPddikti: idPddiktiVal,
            })
            .returning();
          kelas = newKelas;
        } else if (mode === 'update' && idPddiktiVal) {
          await db.update(kelasKuliah).set({ idPddikti: idPddiktiVal }).where(eq(kelasKuliah.id, kelas.id));
        }

        // Lookup dosenId
        const [dsn] = await db.select().from(dosen).where(eq(dosen.nip, nipVal)).limit(1);
        if (!dsn) {
          throw new Error(`Dosen dengan NIP "${nipVal}" tidak ditemukan.`);
        }

        // Check existing dosen pengajar mapping
        const [existingDp] = await db
          .select()
          .from(dosenPengajarKelas)
          .where(and(eq(dosenPengajarKelas.dosenId, dsn.id), eq(dosenPengajarKelas.kelasKuliahId, kelas.id)))
          .limit(1);

        if (existingDp) {
          // Skip silently if already mapped
          result.successCount++;
          continue;
        }
        ...
        await db.insert(dosenPengajarKelas).values({
          dosenId: dsn.id,
          kelasKuliahId: kelas.id,
          sksBebanMengajar: sksVal && !isNaN(sksVal) ? sksVal : null,
          idPddikti: idPddiktiVal,
        });

        result.successCount++;
      } catch (err: any) {
        result.errors.push({ line: lineNum, error: err.message || 'Gagal mengimpor baris.' });
      }
    }
