...
describe('Google Workspace SSO Fail-Fast & Callback Validation', () => {
  ...
  it('harus mengembalikan HTTP status 400 dari POST /auth/google/callback jika code tidak diberikan', async () => {
    const response = await app.handle(
  ...
  it('harus melempar error jika code kosong pada SsoService.handleGoogleCallback', async () => {
    expect(SsoService.handleGoogleCallback('')).rejects.toThrow('Authorization code dari Google wajib diisi');
  });
});
