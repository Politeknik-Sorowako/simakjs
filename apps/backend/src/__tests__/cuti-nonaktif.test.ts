...
import { mahasiswa, periodeAkademik, programStudi, dosen, users } from '../models/schema';
...
describe('Modul Cuti & Mahasiswa Keluar', () => {
  ...
  beforeEach(async () => {
    ...
    await db
      ...
      .values({ id: testPeriodId, nama: 'Semester Ganjil 2024/2025', aktif: true });
  ...
  describe('Pengajuan Cuti', () => {
    ...
    it('Harus berhasil melalui seluruh alur approval cuti', async () => {
      ...
      const [mhsDb] = await db
        .select()
        .from(mahasiswa)
        .where(eq(mahasiswa.id, mhsId));
  ...
  describe('Mahasiswa Keluar (Keluar, DO, dll.)', () => {
    ...
    it('Admin harus berhasil membatalkan pencatatan keluar (Kembali Aktif)', async () => {
      ...
      const keluarRes = await db.insert(mahasiswa).values({
        nim: '20240002',
      ...
      const [mhsDb] = await db
        .select()
        .from(mahasiswa)
        .where(eq(mahasiswa.id, tempMhsId));
