...
SELECT
  ...
  'APEL',
  pa."id",
  (CASE WHEN COALESCE(pa."verified_status"::text, pa."status"::text) = 'telat' THEN 'TERLAMBAT' ELSE UPPER(COALESCE(pa."verified_status"::text, pa."status"::text)) END)::ketidakhadiran_status,
