...
export function Sidebar(props: { isOpen: boolean; onClose: () => void }) {
  ...
  const [isMasterOpen, setIsMasterOpen] = createSignal(true);
  const [isPerencanaanOpen, setIsPerencanaanOpen] = createSignal(true);
  const [isRegistrasiOpen, setIsRegistrasiOpen] = createSignal(true);
  const [isPelaksanaanOpen, setIsPelaksanaanOpen] = createSignal(true);
  const [isEvaluasiOpen, setIsEvaluasiOpen] = createSignal(true);
  const [isLayananOpen, setIsLayananOpen] = createSignal(true);
  const [isIntegrasiOpen, setIsIntegrasiOpen] = createSignal(true);
  ...
  return (
    <aside
      class={`fixed inset-y-0 left-0 z-40 w-64 bg-brand-gray-900 border-r border-brand-gray-800 text-brand-gray-300 min-h-screen flex flex-col shadow-2xl transition-transform duration-300 ease-in-out md:relative md:translate-x-0 ${
        props.isOpen ? 'translate-x-0' : '-translate-x-full'
      }`}
    >
      {/* Brand Header */}
      <div class="h-16 flex items-center justify-between border-b border-brand-gray-800 bg-brand-gray-950 px-6">
        ...
        <button
          ...
          class="p-1 rounded-lg text-brand-gray-400 hover:bg-brand-gray-800 hover:text-white md:hidden focus:outline-none"
        >
      ...
      <nav class="flex-1 p-4 space-y-2 overflow-y-auto">
        <div class="px-3 mb-2 text-[10px] font-semibold text-brand-gray-500 uppercase tracking-widest">Menu Utama</div>

        {/* Dashboard Link */}
        <A
          href="/dashboard"
          onClick={() => props.onClose()}
          activeClass="text-brand-400 font-semibold border-l-2 border-brand-500 pl-2 bg-brand-gray-850/40"
          inactiveClass="hover:bg-brand-gray-800/60 hover:text-white"
          class="flex items-center gap-3 px-3 py-2 rounded-lg text-sm transition-all duration-150"
        >
          <svg
            class="w-5 h-5 text-brand-gray-400 group-hover:text-white transition-colors"
            fill="none"
            viewBox="0 0 24 24"
            stroke="currentColor"
          >
        ...
        <Show when={isAdmin()}>
          <div class="pt-2">
            <button
              ...
              class="w-full flex items-center justify-between px-3 py-2 text-[10px] font-semibold text-brand-gray-500 hover:text-brand-gray-300 uppercase tracking-widest focus:outline-none"
            >
            ...
            <Show when={isMasterOpen()}>
              <div class="mt-1 space-y-1 pl-2 border-l border-brand-gray-800 ml-3">
                <A
                  href="/program-studi"
                  onClick={() => props.onClose()}
                  activeClass="text-brand-400 font-semibold"
                  inactiveClass="hover:text-white text-brand-gray-400"
      ...
      <div class="p-4 border-t border-brand-gray-800 bg-brand-gray-950 flex flex-col gap-3">
        <div class="flex items-center gap-3">
          <Show
          ...
          >
            <img
              ...
              alt="Foto Profil"
              class="h-9 w-9 rounded-full object-cover border border-brand-gray-700 shadow-md"
            />
          </Show>
          <div class="flex-1 overflow-hidden">
            <div class="text-sm font-semibold text-white truncate">{auth.user()?.nama || auth.user()?.email}</div>
            <div class="text-xs text-brand-gray-500 capitalize">{auth.user()?.role}</div>
          </div>
        </div>

        <div class="flex gap-2 text-xs border-t border-brand-gray-800/60 pt-2.5">
          <A
            href="/profil"
            onClick={() => props.onClose()}
            activeClass="bg-brand-gray-800 text-white font-medium"
            class="flex-1 text-center py-1.5 bg-brand-gray-900 hover:bg-brand-gray-800/80 text-brand-gray-300 rounded border border-brand-gray-800 transition-colors"
          >
            Profil
          </A>
          <button
