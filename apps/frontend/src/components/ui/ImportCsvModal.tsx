...
export function ImportCsvModal(props: ImportCsvModalProps) {
	...
	const handleDownloadTemplate = () => {
		const csvContent =
			"data:text/csv;charset=utf-8," + props.templateHeaders.join(",") + "\n";
		const encodedUri = encodeURI(csvContent);
		const link = document.createElement("a");
		link.setAttribute("href", encodedUri);
		link.setAttribute(
			"download",
			`template_${props.title.toLowerCase().replace(/\s+/g, "_")}.csv`,
		);
		...
		try {
			...
			if (res.errors && res.errors.length > 0) {
				if (res.successCount > 0) {
					setSuccessMsg(
						`Berhasil mengimpor ${res.successCount} data (dengan beberapa error).`,
					);
					props.onSuccess();
				} else {
					setErrorMsg(
						"Gagal mengimpor data. Silakan cek detail error di bawah.",
					);
				}
			} else {
				setSuccessMsg(
					`Berhasil mengimpor seluruh data (${res.successCount} baris).`,
				);
				setTimeout(() => {
					props.onSuccess();
					props.onClose();
				...
				<form onSubmit={handleSubmit} class="flex flex-col gap-4">
					<div class="flex flex-col gap-1.5">
						...
						>
							...
							<option value="update">
								Update / Overwrite (Ganti data lama jika data key sudah ada)
							</option>
						</select>
					</div>
					...
					<Show when={errorMsg()}>
						<div class="p-3 bg-red-50 text-red-700 rounded-lg text-xs font-medium">
							{errorMsg()}
						</div>
					</Show>

					<Show when={successMsg()}>
						<div class="p-3 bg-green-50 text-green-700 rounded-lg text-xs font-medium">
							{successMsg()}
						</div>
					</Show>
