                ...
                <div class="flex items-start justify-between">
                  <div class="flex-1">
                    <div class="flex items-center gap-2">
                      ...
                      {item.isWajib ? <span class="text-xs text-red-500 font-semibold">*wajib</span> : <span class="text-xs text-secondary-400">opsional</span>}
                      {item.latest && (
                        <span class={`text-xs px-1.5 py-0.5 rounded-full ${item.latest.isVerified ? 'bg-green-100 text-green-700' : 'bg-yellow-100 text-yellow-700'}`}>
                    ...
                    <div class="text-xs text-secondary-400 mt-0.5">
                      Format: <span class="font-mono">{item.formatFile || 'semua format'}</span> — Maks: {item.maxSizeKb}KB
                    </div>
                        ...
                        <For each={item.uploaded}>
                          {(doc: any) => (
                            <div class="flex items-center justify-between text-xs pl-3 border-l-2 border-secondary-300 dark:border-secondary-600 py-1">
                              <div class="flex items-center gap-2 min-w-0">
                                {doc.uploadMethod === 'link' && doc.fileLink ? (
                                  <a href={doc.fileLink} target="_blank" rel="noopener noreferrer"
                                    class="text-brand-600 hover:text-brand-700 underline truncate">🔗 Buka Link Google Drive</a>
                                ) : doc.filePath ? (
                                  <div class="flex items-center gap-2">
                                    <a href={`${import.meta.env.VITE_API_URL || 'http://localhost:3000'}/admisi/documents/${doc.id}/file`}
                                      target="_blank" rel="noopener noreferrer"
                                      class="text-brand-600 hover:text-brand-700 underline truncate">📄 {doc.originalName || 'Lihat File'}</a>
                              ...
                              <button onClick={() => handleDelete(doc.id)} class="text-red-500 hover:text-red-700 flex-shrink-0 ml-2">Hapus</button>
                            </div>
                          )}
                        </For>
                      </div>
                  ...
                  <div class="flex gap-2 ml-4 flex-shrink-0">
                    ...
                    <Button size="sm" variant="secondary"
                      onClick={() => setShowLinkInput(showLinkInput() === item.id ? null : item.id)}>Link</Button>
                  </div>
                </div>

                <Show when={showLinkInput() === item.id}>
                  <div class="mt-3 flex gap-2">
                    <input type="url" placeholder="Tempel link Google Drive..." value={linkValue()}
                    ...
                    <Button size="sm" onClick={() => handleLinkSubmit(item.id)} disabled={!linkValue()}>Kirim</Button>
                  </div>
                </Show>
              </div>
            )}
