      ...
      <div class="flex flex-col gap-6 max-w-6xl mx-auto p-4">
        ...
        <div class="flex flex-col md:flex-row md:items-center justify-between gap-4 bg-white p-6 rounded-2xl border border-brand-gray-100 shadow-sm">
          ...
          <button
            onClick={handleSync}
            disabled={isSyncing()}
            class="px-5 py-2.5 bg-brand-600 hover:bg-brand-700 text-white font-extrabold text-sm rounded-xl disabled:opacity-50 active:scale-95 transition-all shadow-md shadow-blue-100 flex items-center gap-2"
          >
        ...
        <Show
          when={stats()}
          fallback={<div class="text-center py-12 text-brand-gray-400">Memuat status sinkronisasi...</div>}
        >
          <div class="grid grid-cols-1 md:grid-cols-3 gap-6">
            ...
            <div class="bg-white p-6 rounded-2xl border border-brand-gray-100 shadow-sm flex flex-col gap-4">
              ...
              <div class="grid grid-cols-2 gap-2 border-t pt-3 text-xs">
                <div>
                  ...
                  <p class="font-bold text-accent-600">{stats()?.mahasiswa?.synced}</p>
                </div>
                <div>
            ...
            <div class="bg-white p-6 rounded-2xl border border-brand-gray-100 shadow-sm flex flex-col gap-4">
              ...
              <div class="grid grid-cols-2 gap-2 border-t pt-3 text-xs">
                <div>
                  <p class="text-brand-gray-400">Tersinkron</p>
                  <p class="font-bold text-accent-600">{stats()?.kelasKuliah?.synced}</p>
                </div>
                <div>
                  <p class="text-brand-gray-400">Belum Sinkron</p>
            ...
            <div class="bg-white p-6 rounded-2xl border border-brand-gray-100 shadow-sm flex flex-col gap-4">
              ...
              <div class="grid grid-cols-2 gap-2 border-t pt-3 text-xs">
                <div>
                  <p class="text-brand-gray-400">Tersinkron</p>
                  <p class="font-bold text-accent-600">{stats()?.krs?.synced}</p>
                </div>
                <div>
                  <p class="text-brand-gray-400">Belum Sinkron</p>
        ...
        <Show when={syncLogs().length > 0}>
          <div class="bg-brand-gray-900 rounded-2xl p-6 border border-brand-gray-800 shadow-xl flex flex-col gap-4 font-mono text-xs text-brand-gray-300">
            ...
            <div class="flex flex-col gap-2 max-h-72 overflow-y-auto">
              <For each={syncLogs()}>
                {(log) => (
                  <div
                    class={`${log.startsWith('❌') ? 'text-rose-400' : log.startsWith('✅') ? 'text-accent-450 font-bold' : 'text-brand-gray-300'}`}
                  >
                    {log}
                  </div>
                )}
