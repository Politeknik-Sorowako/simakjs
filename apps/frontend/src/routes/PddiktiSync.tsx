      ...
      <div class="flex flex-col gap-6 max-w-6xl mx-auto p-4">
        ...
        <div class="flex flex-col md:flex-row md:items-center justify-between gap-4 bg-white p-6 rounded-2xl border border-gray-100 shadow-sm">
          ...
          <button
            onClick={handleSync}
            disabled={isSyncing()}
            class="px-5 py-2.5 bg-blue-600 hover:bg-blue-700 text-white font-extrabold text-sm rounded-xl disabled:opacity-50 active:scale-95 transition-all shadow-md shadow-blue-100 flex items-center gap-2"
          >
          ...
          <div class="grid grid-cols-1 md:grid-cols-3 gap-6">
            ...
            <div class="bg-white p-6 rounded-2xl border border-gray-100 shadow-sm flex flex-col gap-4">
              <div class="flex justify-between items-center">
                <span class="text-xs uppercase font-extrabold tracking-wider text-gray-400">Data Mahasiswa</span>
                <span class="px-2 py-0.5 bg-blue-50 text-blue-600 text-[10px] font-bold rounded">PDDIKTI</span>
              </div>
              <div class="flex flex-col gap-1">
                <span class="text-3xl font-extrabold text-gray-800">{stats()?.mahasiswa?.total}</span>
                <span class="text-xs text-gray-450 font-medium">Total Mahasiswa Terdaftar</span>
              </div>
              <div class="grid grid-cols-2 gap-2 border-t pt-3 text-xs">
                <div>
                  <p class="text-gray-400">Tersinkron</p>
                  <p class="font-bold text-emerald-600">{stats()?.mahasiswa?.synced}</p>
                </div>
                <div>
                  <p class="text-gray-400">Belum Sinkron</p>
                  <p class="font-bold text-rose-500">{stats()?.mahasiswa?.unsynced}</p>
                </div>
              </div>
            </div>
            ...
            <div class="bg-white p-6 rounded-2xl border border-gray-100 shadow-sm flex flex-col gap-4">
              <div class="flex justify-between items-center">
                <span class="text-xs uppercase font-extrabold tracking-wider text-gray-400">Kelas Kuliah</span>
                <span class="px-2 py-0.5 bg-indigo-50 text-indigo-600 text-[10px] font-bold rounded">PDDIKTI</span>
              </div>
              <div class="flex flex-col gap-1">
                <span class="text-3xl font-extrabold text-gray-800">{stats()?.kelasKuliah?.total}</span>
                <span class="text-xs text-gray-450 font-medium">Total Kelas Kuliah</span>
              </div>
              <div class="grid grid-cols-2 gap-2 border-t pt-3 text-xs">
                <div>
                  <p class="text-gray-400">Tersinkron</p>
                  <p class="font-bold text-emerald-600">{stats()?.kelasKuliah?.synced}</p>
                </div>
                <div>
                  <p class="text-gray-400">Belum Sinkron</p>
                  <p class="font-bold text-rose-500">{stats()?.kelasKuliah?.unsynced}</p>
                </div>
              </div>
            </div>
            ...
            <div class="bg-white p-6 rounded-2xl border border-gray-100 shadow-sm flex flex-col gap-4">
              <div class="flex justify-between items-center">
                <span class="text-xs uppercase font-extrabold tracking-wider text-gray-400">KRS & Nilai</span>
                <span class="px-2 py-0.5 bg-violet-50 text-violet-600 text-[10px] font-bold rounded">PDDIKTI</span>
              </div>
              <div class="flex flex-col gap-1">
                <span class="text-3xl font-extrabold text-gray-800">{stats()?.krs?.total}</span>
                <span class="text-xs text-gray-450 font-medium">Total KRS Terkontrak</span>
              </div>
              <div class="grid grid-cols-2 gap-2 border-t pt-3 text-xs">
                <div>
                  <p class="text-gray-400">Tersinkron</p>
                  <p class="font-bold text-emerald-600">{stats()?.krs?.synced}</p>
                </div>
                <div>
                  <p class="text-gray-400">Belum Sinkron</p>
                  <p class="font-bold text-rose-500">{stats()?.krs?.unsynced}</p>
                </div>
              </div>
            </div>
          </div>
        ...
        <Show when={syncLogs().length > 0}>
          <div class="bg-gray-900 rounded-2xl p-6 border border-gray-800 shadow-xl flex flex-col gap-4 font-mono text-xs text-gray-300">
            <div class="flex justify-between items-center border-b border-gray-800 pb-2">
              <span class="text-gray-400 font-bold uppercase tracking-wider">Sync Log Console</span>
