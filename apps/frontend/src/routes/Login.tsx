  ...
  const handleGoogleSSO = async () => {
    ...
    try {
      ...
      if (!res.url || res.url.trim() === '') {
        throw new Error('URL autentikasi Google SSO tidak valid');
      }
      ...
      const msg =
        e instanceof Error && e.message
          ? e.message
          : 'Konfigurasi Google Workspace SSO belum lengkap di server. Silakan hubungi Administrator.';
      setErrorMsg(msg);
      ...
      <div class="w-full max-w-md bg-white dark:bg-secondary-900/60 dark:backdrop-blur-xl border border-secondary-200/80 dark:border-white/10 p-8 rounded-2xl shadow-xl dark:shadow-2xl flex flex-col gap-6 relative z-10 text-secondary-800 dark:text-white transition-all duration-200">
        ...
        <Show when={!isRegister()}>
          <div class="flex items-center gap-3 my-1">
            <div class="h-px flex-1 bg-secondary-200 dark:bg-white/10" />
            <span class="text-xs text-secondary-400 dark:text-secondary-500 font-medium">atau</span>
            <div class="h-px flex-1 bg-secondary-200 dark:bg-white/10" />
          </div>
          ...
          <button
            ...
            disabled={loading()}
            class="w-full flex items-center justify-center gap-3 py-2.5 px-4 rounded-xl border border-secondary-200 dark:border-white/15 bg-white dark:bg-secondary-800/80 text-secondary-700 dark:text-white hover:bg-secondary-50 dark:hover:bg-secondary-800 font-semibold text-sm transition-all focus:outline-none focus:ring-2 focus:ring-primary-500/30 disabled:opacity-50"
          >
