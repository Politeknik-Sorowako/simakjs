...
export default function AdmisiVABanks() {
  ...
  const handleSubmit = async (e: Event) => {
    ...
    try {
      ...
      setShowForm(false); setKode(''); setNama(''); setIsMidtrans(false);
      refetch();
          ...
          <form onSubmit={handleSubmit} class="bg-white dark:bg-secondary-800/40 border border-secondary-200 dark:border-secondary-700 rounded-xl p-5 mb-6 space-y-4">
            <div class="flex flex-wrap gap-2 mb-2">
              ...
              <For each={presetBanks}>
                {(p) => (
                  <button type="button" onClick={() => { setKode(p.kode); setNama(p.nama); }}
                    class="text-xs px-2 py-1 bg-secondary-100 dark:bg-secondary-700 rounded hover:bg-secondary-200">{p.nama}</button>
                )}
              </For>
            </div>
            <div class="grid md:grid-cols-2 gap-4">
              <div>
                ...
                <input required value={kode()} onInput={(e) => setKode(e.currentTarget.value)}
                  class="w-full px-3 py-2 border border-secondary-300 rounded-lg text-sm bg-white dark:bg-secondary-800" placeholder="bca" />
              </div>
              <div>
                <label class="text-sm font-medium block mb-1">Nama Bank</label>
                <input required value={nama()} onInput={(e) => setNama(e.currentTarget.value)}
                  class="w-full px-3 py-2 border border-secondary-300 rounded-lg text-sm bg-white dark:bg-secondary-800" placeholder="Bank BCA" />
              </div>
            </div>
            <label class="flex items-center gap-2 text-sm">
              <input type="checkbox" checked={isMidtrans()} onChange={(e) => setIsMidtrans(e.currentTarget.checked)}
              ...
              <div class="bg-white dark:bg-secondary-800/40 border border-secondary-200 dark:border-secondary-700 rounded-xl p-4 flex items-center justify-between">
                <div>
                  <div class="flex items-center gap-2">
                    ...
                    <span class={`text-xs px-2 py-0.5 rounded-full ${b.isActive ? 'bg-green-100 text-green-700' : 'bg-gray-100 text-gray-500'}`}>{b.isActive ? 'Aktif' : 'Nonaktif'}</span>
                    {b.isMidtrans && <span class="text-xs px-2 py-0.5 bg-purple-100 text-purple-700 rounded-full">Midtrans</span>}
                  </div>
                ...
                <div class="flex gap-2">
                  ...
                  <button onClick={() => handleDelete(b.id)} class="text-xs px-3 py-1 rounded border border-red-300 text-red-600">Hapus</button>
                </div>
              </div>
            )}
          </For>
