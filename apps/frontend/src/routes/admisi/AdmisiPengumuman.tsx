          ...
          >
            ...
            <div>
              ...
              <textarea
                required
                value={isi()}
                onInput={(e) => setIsi(e.currentTarget.value)}
                rows={4}
                class="w-full px-3 py-2 border border-secondary-300 dark:border-secondary-600 rounded-lg bg-white dark:bg-secondary-800 text-sm"
              />
            </div>
            <div>
              <label class="text-sm font-medium block mb-1">
                Lampiran {editingId() !== null ? '(pilih file untuk ganti lampiran lama)' : '(opsional)'}
              </label>
              <input
                type="file"
                ...
                <div class="flex items-start justify-between">
                  <div class="flex-1">
                    <div class="flex items-center gap-2">
                      {a.isPinned ? (
                        <span class="text-xs px-1.5 py-0.5 bg-brand-100 dark:bg-brand-900/30 text-brand-700 dark:text-brand-300 rounded font-semibold">
                          PINNED
                        </span>
                      ) : null}
                  ...
                  <div class="flex gap-2 ml-4 flex-shrink-0">
                    <button onClick={() => handleEdit(a)} class="text-xs text-brand-600 hover:text-brand-700">
                      Edit
                    </button>
                    <button onClick={() => handleDelete(a.id)} class="text-xs text-red-500 hover:text-red-700">
                      Hapus
                    </button>
                  </div>
                </div>
