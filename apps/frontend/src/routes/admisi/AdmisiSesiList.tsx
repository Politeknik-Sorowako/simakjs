    ...
    <MainLayout>
      <div class="p-4 md:p-6 max-w-6xl mx-auto">
        <div class="flex items-center justify-between mb-6">
          ...
          <Button onClick={() => setShowForm(!showForm())}>{showForm() ? 'Batal' : '+ Sesi Baru'}</Button>
        </div>
        ...
        <Show when={sessions()}>
          {(() => {
            ...
            if (akanDatang.length)
              sections.push({ label: 'Akan Datang', icon: '🟡', color: 'border-l-amber-400', items: akanDatang });
            if (nonaktif.length)
              sections.push({ label: 'Nonaktif', icon: '⚪', color: 'border-l-gray-400', items: nonaktif });
            if (ditutup.length)
              sections.push({ label: 'Sudah Ditutup', icon: '🔴', color: 'border-l-red-400', items: ditutup });
                      ...
                      <div class="grid gap-3">
                        <For each={sec.items}>
                          {(session: any) => {
                            ...
                            return (
                              <div
                                class={`bg-white dark:bg-secondary-800/40 border border-secondary-200 dark:border-secondary-700 rounded-xl p-4 border-l-4 ${sec.color}`}
                              >
                                <div class="flex items-center justify-between">
                                  <div class="flex-1">
                                    <div class="flex items-center gap-2">
                                      ...
                                      <span
                                        class={`text-xs px-2 py-0.5 rounded-full ${visibleToPublic ? 'bg-brand-100 text-brand-700' : 'bg-gray-100 text-gray-400'}`}
                                      >
                                    ...
                                    <div class="text-xs text-secondary-400 mt-0.5">
                                      ...
                                      —{' '}
                                      {tutup.toLocaleDateString('id-ID', {
                                        day: 'numeric',
                                        month: 'long',
                                        year: 'numeric',
                                      })}
                                  ...
                                  <div class="flex gap-2 flex-shrink-0 ml-4">
                                    <Show when={sec.label !== 'Nonaktif'}>
                                      <button
                                        onClick={() => handleToggleActive(session.id, true)}
                                        class="text-xs px-2 py-1 rounded border border-amber-300 text-amber-600 hover:bg-amber-50"
                                      >
                                        Nonaktifkan
                                      </button>
                                    </Show>
                                    <Show when={sec.label === 'Nonaktif'}>
                                      <button
                                        onClick={() => handleToggleActive(session.id, false)}
                                    ...
                                    <A
                                      ...
                                      class="text-xs px-2 py-1 rounded border border-brand-300 text-brand-600 hover:bg-brand-50"
                                    >
                                      Detail
                                    </A>
                                  </div>
