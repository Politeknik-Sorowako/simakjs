    ...
    <MainLayout>
      <div class="flex flex-col gap-6">
        ...
        <div class="flex flex-col md:flex-row md:items-center justify-between gap-4 bg-white p-6 rounded-2xl border border-brand-gray-100 shadow-sm">
          <div>
            <h1 class="text-2xl font-extrabold text-brand-gray-800 tracking-tight">Kedisiplinan Mahasiswa</h1>
            <p class="text-sm text-brand-gray-500">Pencatatan pelanggaran indisipliner dan rekap poin kedisiplinan</p>
          </div>
        ...
        <Show when={user()?.role === 'mahasiswa'}>
          <div class="grid grid-cols-1 lg:grid-cols-3 gap-6">
            {/* Summary Widget */}
            <div class="bg-white border border-brand-gray-100 rounded-2xl p-6 shadow-sm flex flex-col gap-4">
              <h3 class="font-bold text-brand-gray-800 border-b pb-2">Status Kedisiplinan</h3>
            ...
            {/* Violation List */}
            <div class="lg:col-span-2 bg-white border border-brand-gray-100 rounded-2xl shadow-sm overflow-hidden p-6 flex flex-col gap-4">
              <h3 class="font-bold text-brand-gray-800 border-b pb-2">Riwayat Tindakan Indisipliner</h3>
              <Show
                when={studentViolations()?.pelanggaranList && studentViolations()!.pelanggaranList.length > 0}
                fallback={
                  <div class="py-12 text-center text-brand-gray-400 text-sm">
        ...
        {/* Admin & Dosen View */}
        <Show when={user()?.role === 'admin' || user()?.role === 'dosen'}>
          <div class="bg-white border border-brand-gray-100 rounded-2xl shadow-sm p-6 flex flex-col gap-4">
            <h3 class="font-bold text-brand-gray-800 border-b pb-2">Daftar Pelanggaran Mahasiswa</h3>
            <Show
        ...
        <Modal show={showModal()} onClose={() => setShowModal(false)} title={editPelanggaranId() ? "Edit Catatan Pelanggaran" : "Catat Pelanggaran Baru"}>
          <form onSubmit={handleSave} class="flex flex-col gap-4">
            ...
            {/* Bobot Poin */}
            <div class="flex flex-col gap-1">
              <label class="text-xs font-bold text-brand-gray-700">Bobot Pelanggaran (Poin)</label>
              <input
                type="number"
                min="1"
                max="100"
            ...
            {/* Keterangan */}
            <div class="flex flex-col gap-1">
              <label class="text-xs font-bold text-brand-gray-700">Keterangan Detail</label>
              <textarea
                rows="4"
