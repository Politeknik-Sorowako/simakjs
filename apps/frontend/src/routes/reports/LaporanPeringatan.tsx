...
export default function LaporanPeringatan() {
  ...
  const [rekapPelanggaran] = createResource(
    async () => {
      try {
      ...
      } catch { return { total: 0, totalPoin: 0, perJenis: [], data: [] }; }
    },
  );
      ...
      <div class="flex flex-col gap-6">
        <div class="flex flex-col md:flex-row justify-between items-start md:items-center gap-4">
          <div>
            ...
            <p class="text-sm text-secondary-500 dark:text-secondary-200">Rekapitulasi pelanggaran dan kedisiplinan mahasiswa</p>
          </div>
          <ExportButtonGroup
        ...
        <div class="grid grid-cols-1 sm:grid-cols-3 gap-4">
          ...
          <StatCard
            ...
            color="yellow"
            icon={<svg class="w-7 h-7" fill="none" viewBox="0 0 24 24" stroke="currentColor"><path stroke-linecap="round" stroke-linejoin="round" stroke-width="2" d="M13 17h8m0 0V9m0 8l-8-8-4 4-6-6" /></svg>}
          />
          <StatCard
            title="Rata-rata Poin"
            value={rekapPelanggaran()?.total ? (rekapPelanggaran()!.totalPoin / rekapPelanggaran()!.total).toFixed(1) : '0'}
        ...
        <Show when={(rekapPelanggaran()?.perJenis?.length || 0) > 0}>
          <div class="grid grid-cols-1 lg:grid-cols-2 gap-6">
            <div class="bg-white dark:bg-secondary-900 border border-secondary-100 dark:border-secondary-800 p-5 rounded-2xl shadow-sm">
              <h3 class="text-sm font-bold text-secondary-800 dark:text-white mb-3">Distribusi Pelanggaran per Jenis</h3>
              <PieChart
        ...
        <div class="bg-white dark:bg-secondary-900 border border-secondary-100 dark:border-secondary-800 rounded-2xl shadow-sm overflow-hidden">
          ...
          <div class="overflow-x-auto">
            <table class="w-full text-left text-xs border-collapse">
              ...
              <tbody>
                <For each={rekapPelanggaran()?.data || []} fallback={<tr><td colspan="4" class="text-center py-8 text-secondary-400">Belum ada data pelanggaran</td></tr>}>
                  {(item: any) => (
                    <tr class="border-b border-secondary-50 hover:bg-secondary-50/30 dark:hover:bg-secondary-800/30">
                      <td class="py-3 px-5">{new Date(item.tanggal).toLocaleDateString('id-ID')}</td>
                      <td class="py-3 px-5 font-semibold text-secondary-800 dark:text-white">{item.jenisPelanggaran}</td>
