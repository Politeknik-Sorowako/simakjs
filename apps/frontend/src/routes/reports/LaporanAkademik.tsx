...
export default function LaporanAkademik() {
  ...
  const [rekap] = createResource(
    ...
    async (periodeId) => {
      ...
      try { return await khsController.getRekapPerProdi(periodeId); }
      catch { return null; }
    },
  );
      ...
      <div class="flex flex-col gap-6">
        <div class="flex flex-col md:flex-row justify-between items-start md:items-center gap-4">
          <div>
            ...
            <p class="text-sm text-secondary-500 dark:text-secondary-200">Rata-rata IP dan performa akademik per prodi</p>
          </div>
          <Show when={rekap()}>
            <ExportButtonGroup data={() => rekap()?.prodi || []} columns={columns} filename={'Akademik_' + selectedPeriode()} title="Laporan Akademik per Prodi" subtitle={'Periode: ' + selectedPeriode()} />
          </Show>
        </div>
        ...
        <Show when={rekap()}>
          <div class="grid grid-cols-1 lg:grid-cols-2 gap-6">
            <div class="bg-white dark:bg-secondary-900 border border-secondary-100 dark:border-secondary-800 p-5 rounded-2xl shadow-sm">
              ...
              <BarChart labels={(rekap()?.prodi || []).map((p: any) => p.prodiNama)} datasets={[{ label: 'Rata-rata IP', data: (rekap()?.prodi || []).map((p: any) => p.rataIP), backgroundColor: '#6366f1' }]} height={300} horizontal />
            </div>
            <div class="bg-white dark:bg-secondary-900 border border-secondary-100 dark:border-secondary-800 p-5 rounded-2xl shadow-sm">
              <h3 class="text-sm font-bold text-secondary-800 dark:text-white mb-3">Jumlah Mahasiswa per Prodi</h3>
              <BarChart labels={(rekap()?.prodi || []).map((p: any) => p.prodiNama)} datasets={[{ label: 'Total Mahasiswa', data: (rekap()?.prodi || []).map((p: any) => p.totalMahasiswa), backgroundColor: '#06b6d4' }]} height={300} horizontal />
            </div>
          </div>
        </Show>
        ...
        <div class="bg-white dark:bg-secondary-900 border border-secondary-100 dark:border-secondary-800 rounded-2xl shadow-sm overflow-hidden">
          <div class="px-5 py-3 border-b border-secondary-100 dark:border-secondary-800"><h3 class="text-sm font-bold text-secondary-800 dark:text-white">Detail per Program Studi</h3></div>
