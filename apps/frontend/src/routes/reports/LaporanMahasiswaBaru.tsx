...
export default function LaporanMahasiswaBaru() {
  ...
  const [stats] = createResource(() => mahasiswaController.getMahasiswaBaru(angkat()));
  ...
  const trendColumns: ExportColumn[] = [
    ...
    { header: 'Total Mahasiswa', accessor: 'total' },
  ];
    ...
    <MainLayout>
      <div class="flex flex-col gap-6">
        ...
        <div class="grid grid-cols-1 sm:grid-cols-3 gap-4">
          <StatCard title="Total Mahasiswa Baru" value={stats()?.total || '...'} color="brand"
          ...
          />
          <StatCard title="Perempuan" value={stats()?.perProdi.reduce((s: number, p: any) => s + p.perempuan, 0) || '...'} color="accent"
          ...
          />
          <StatCard title="Laki-laki" value={stats()?.perProdi.reduce((s: number, p: any) => s + p.laki, 0) || '...'} color="green"
            icon={<svg class="w-7 h-7" fill="none" viewBox="0 0 24 24" stroke="currentColor"><path stroke-linecap="round" stroke-linejoin="round" stroke-width="2" d="M16 7a4 4 0 11-8 0 4 4 0 018 0zM12 14a7 7 0 00-7 7h14a7 7 0 00-7-7z" /></svg>}
          />
        </div>
