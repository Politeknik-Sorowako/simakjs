...
import { StatCard, PieChart, BarChart, LineChart } from '../components/charts';
    ...
    <div class="flex flex-col gap-6">
      ...
      <div class="grid grid-cols-1 lg:grid-cols-2 gap-6">
        ...
        <div class="bg-white dark:bg-secondary-900 border border-secondary-100 dark:border-secondary-800 p-5 rounded-2xl shadow-sm">
          ...
          <Show when={tagihanStats().total > 0} fallback={
            <p class="text-xs text-secondary-400 text-center py-8">Belum ada data tagihan</p>
          }>
            <div class="grid grid-cols-1 sm:grid-cols-2 gap-4">
              ...
              <div class="flex flex-col justify-center gap-3">
                <div class="flex justify-between text-xs">
                  ...
                  <span class="font-bold text-secondary-800 dark:text-white">{formatRupiah(tagihanStats().totalNominal)}</span>
                </div>
                <div class="flex justify-between text-xs">
                ...
                <div class="flex justify-between text-xs">
                  ...
                  <span class="font-bold text-rose-600">{formatRupiah(tagihanStats().totalNominal - tagihanStats().totalTerbayar)}</span>
                </div>
        ...
        <div class="bg-white dark:bg-secondary-900 border border-secondary-100 dark:border-secondary-800 p-5 rounded-2xl shadow-sm">
          ...
          <Show when={!pddiktiStats.loading && pddiktiStats()} fallback={<p class="text-xs text-secondary-400 text-center py-8">Memuat...</p>}>
            <div class="space-y-3">
              <For each={Object.entries(pddiktiStats() || {})}>
                {([key, val]: [string, any]) => (
                  <div class="flex items-center justify-between border-b border-secondary-100 dark:border-secondary-800 pb-2 last:border-0">
                    <span class="text-xs font-semibold text-secondary-600 dark:text-secondary-300 capitalize">{key}</span>
                    <div class="flex items-center gap-2">
                      <span class="text-xs text-secondary-400">{val.total} total</span>
                      <span class={`inline-flex items-center px-2 py-0.5 rounded-full text-[10px] font-bold ${
                        val.unsynced > 0 ? 'bg-yellow-50 text-yellow-700 border border-yellow-200' : 'bg-green-50 text-green-700 border border-green-200'
                      }`}>
      ...
      <div class="bg-gradient-to-r from-brand-600 to-accent-700 text-white rounded-2xl p-5 shadow-sm">
        <div class="flex flex-col sm:flex-row justify-between items-start sm:items-center gap-2">
          ...
          <a href="/periode-akademik" class="px-4 py-2 text-xs font-bold bg-white/20 hover:bg-white/30 rounded-xl transition-colors">
            Kelola Periode
          </a>
        </div>
      </div>
      ...
      <div class="bg-white dark:bg-secondary-900 border border-secondary-100 dark:border-secondary-800 rounded-2xl p-5 shadow-sm">
        <div class="flex justify-between items-center mb-4">
          <h3 class="text-sm font-bold text-secondary-800 dark:text-white">
            ...
            <span class="ml-2 text-[10px] font-normal text-secondary-400">
                ...
                <For each={dosenBimbingan()}>
                  {(m: any) => (
                    <tr class="border-b border-secondary-50 dark:border-secondary-800/50 hover:bg-secondary-50/50 dark:hover:bg-secondary-800/30">
                      ...
                      <td class="py-3">
                        <a href={`/bimbingan?mhs=${m.id}`} class="text-brand-600 hover:text-brand-700 font-bold text-[10px]">
                          Detail →
                        </a>
                      </td>
                    </tr>
...
function MahasiswaWidgets() {
  ...
  const [akademikSummary] = createResource(
    mhsId,
    async (id) => {
      if (!id) return null;
      try {
  ...
  const [transkrip] = createResource(
    mhsId,
    async (id) => {
      if (!id) return null;
      try {
  ...
  const [tagihanMhs] = createResource(
    nim,
    async (n) => {
      if (!n) return { data: [] };
    ...
    <div class="flex flex-col gap-6">
          ...
          <Show
            ...
            fallback={
              <div class="text-center py-8">
                ...
                <a href="/keuangan" class="text-xs font-bold text-brand-600 hover:text-brand-700 underline mt-2 inline-block">
                  Lihat Riwayat →
                </a>
              </div>
            }
          >
            <div class="space-y-4">
              <div class="flex items-center justify-between text-xs">
                ...
                <span class="font-bold text-secondary-800 dark:text-white">{formatRupiah(currentTagihan()?.nominal || 0)}</span>
              </div>
              <div class="flex items-center justify-between text-xs">
              ...
              <div class="flex items-center justify-between text-xs">
                ...
                <span class="font-bold text-rose-600">{formatRupiah((currentTagihan()?.nominal || 0) - (currentTagihan()?.nominalTerbayar || 0))}</span>
              </div>
              <div class="bg-secondary-50 dark:bg-secondary-800 rounded-xl h-2 overflow-hidden">
                <div
                  ...
                  style={{ width: `${Math.min(100, ((currentTagihan()?.nominalTerbayar || 0) / (currentTagihan()?.nominal || 1)) * 100)}%` }}
                />
              </div>
              <div class="text-center">
                <span class={`inline-flex items-center px-2.5 py-0.5 rounded-full text-xs font-semibold ${
                  currentTagihan()?.status === 'lunas' ? 'bg-green-50 text-green-700 border border-green-200' :
                  currentTagihan()?.status === 'cicilan' ? 'bg-brand-50 text-brand-700 border border-brand-200' :
                  'bg-yellow-50 text-yellow-700 border border-yellow-200'
                }`}>
                  {currentTagihan()?.status === 'lunas' ? 'Lunas' : currentTagihan()?.status === 'cicilan' ? 'Cicilan' : 'Belum Bayar'}
                </span>
              </div>
            </div>
          </Show>
      ...
      <div class="bg-gradient-to-r from-rose-500 to-rose-600 text-white rounded-2xl p-5 shadow-sm">
        <div class="flex items-center justify-between">
          ...
          <a href="/pelanggaran" class="px-4 py-2 text-xs font-bold bg-white/20 hover:bg-white/30 rounded-xl transition-colors">
            Lihat Detail
          </a>
        </div>
      </div>
        ...
        <div class="bg-gradient-to-r from-brand-600 to-accent-700 text-white rounded-2xl p-8 shadow-lg relative overflow-hidden">
          ...
          <div class="relative z-10 flex flex-col gap-2">
            ...
            <p class="text-brand-100 max-w-xl">
              Anda masuk sebagai <strong class="uppercase text-white">{role()}</strong> di SIMAK Vokasi Politeknik Sorowako.
            </p>
          </div>
        </div>

        {/* Role-based Dashboard Content */}
        <Show when={role() === 'admin'}><AdminWidgets /></Show>
        <Show when={role() === 'dosen'}><DosenWidgets /></Show>
        <Show when={role() === 'mahasiswa'}><MahasiswaWidgets /></Show>
        ...
        <Show when={role() === 'prodi' || role() === 'keuangan'}>
          <div class="bg-white dark:bg-secondary-900 border border-secondary-100 dark:border-secondary-800 rounded-2xl p-6 shadow-sm">
            <p class="text-sm text-secondary-500">
              Dashboard khusus untuk role <strong>{role()}</strong> sedang dalam pengembangan. Silakan gunakan menu navigasi di sebelah kiri.
            </p>
          </div>
        </Show>
      </div>
